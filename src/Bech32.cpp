...
namespace Bech32
{
    ...
    namespace
    {
        ...
        uint32_t EncodingConstant(Encoding encoding) {
            if (!(encoding == Encoding::BECH32 || encoding == Encoding::BECH32M)) throw std::exception();   //invalid encoding
 ...
 *  bits correspond to earlier values. */
        ...
        Encoding VerifyChecksum(const std::string& hrp, const data& values)
        {
            ...
            const uint32_t check = PolyMod(utils::concatenate(ExpandHRP(hrp), values));
        ...
        {
            data enc = utils::concatenate(ExpandHRP(hrp), values);
    ...
    std::string Encode(Encoding encoding, const std::string& hrp, const data& values) {
        ...
        for (const char& c : hrp) {
            if (!(c < 'A' || c > 'Z')) throw std::exception();
        }
        data checksum = CreateChecksum(encoding, hrp, values);
        data combined = utils::concatenate(values, checksum);
