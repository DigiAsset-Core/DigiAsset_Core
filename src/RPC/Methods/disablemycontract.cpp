...
namespace RPC {
    namespace Methods {
        /**
        * Disable a contract that is enabled(if already disabled does nothing)
        * Note: only works on your contracts
        * params[0] - pluginName(string)
        * params[1] - contractAddress(string)
        */
        extern const Response disablemycontract(const Json::Value& params) {
            ...
            //get list of plugins
            bool success = contract->disableContract(params[1].asString());
